function addImplementationPlanSheet() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();

  // 既存シートがあれば削除して作り直す
  var existingSheet = ss.getSheetByName("実装計画");
  if (existingSheet) {
    ss.deleteSheet(existingSheet);
  }

  var sheet = ss.insertSheet("実装計画");
  ...
  var rows = [
    ...
    ["・バリデーション: 退社時間 > 出社時間 / 退社・出社どちらか一方のみは不可 / 指示者確認欄が空欄の行はエラーチェック対象外"],
  ...
  SpreadsheetApp.getUi().alert("「実装計画」シートを追加しました。");
}
